...
public class DragRectangle extends JFrame implements MouseListener, MouseMotionListener {
	...
	private class DrawingPanel extends JPanel{
		...
		//	g.setColor(backColour);
			...
			// Grabbing minimum value, because you can't draw rectangles backwards, Similar to math.min & math.rad
			int xorigin, yorigin, mh, mw;
...
//TODO:			ADD CODE HERE for different directions
			g.drawRect(xorigin,yorigin,mw,mh);
			
			if (dragging) {
				// Copying the previous values to a backup variable
				prevx = xorigin; prevy = yorigin;
				prevw = mw;  prevh = mh;
			} else {
				prevx = prevy = 0;
				prevw = prevh = 0;
